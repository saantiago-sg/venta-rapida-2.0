-- Restaura el descuento de stock de componentes de combo en process_sale.
--
-- La logica de expansion de combo (insertar en sale_item_components y descontar stock de cada
-- componente real, ver el process_sale original en 20260814181500_combo_products.sql) se perdio
-- al reescribir la funcion completa en 20260902210000_process_sale_price_tolerance.sql para
-- agregar la tolerancia de precio de ventas offline -- esa reescritura copio el resto de la
-- logica pero se olvido de la rama de combos. Desde entonces, vender un combo registra la venta
-- bien pero nunca descuenta stock de sus componentes.
--
-- cancel_sale (ver 20260818190000_electronic_invoicing.sql) SI sigue teniendo la reversion de
-- combos intacta -- estaba "dormida" nada mas, porque nunca se generaban filas en
-- sale_item_components para revertir. No hace falta tocarla.
--
-- Mismo signature y logica de calculo que la version actual (20260908123000_cash_register_toggle.sql)
-- -- lo unico que cambia es el segundo loop, que vuelve a distinguir combo vs producto simple.
...
begin
  ...
  loop
    ...
    if v_product.is_combo then
      ...
      -- aunque un componente quede en negativo -- mismo criterio de "nunca bloquear la venta
      -- por falta de stock" que ya rige el resto del catalogo.
      for v_component in
